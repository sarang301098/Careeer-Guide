...
//            	}
            ...
            String objId="resultGraph";
            request.getSession().setAttribute(objId, h);
            System.out.println("ans:+"+h);
        	request.setAttribute("Result", objId);
        	request.getRequestDispatcher("NextPage.jsp").forward(request, response);
        }
        else 
        {
        	response.sendRedirect("NextQuestion.jsp");
        }
